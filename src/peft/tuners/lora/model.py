    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        target_name_key = next(
            filter(lambda key: re.match(f".*\.{key}$", current_key), pattern_keys),
            current_key,
        )
        ...
        kwargs = {
            ...
            "loaded_in_8bit": optional_kwargs.pop("loaded_in_8bit", False),
            "loaded_in_4bit": optional_kwargs.pop("loaded_in_4bit", False),
            "bias": bias,
        }
            ...
            if isinstance(target, LoraLayer):
                ...
                if combination_type == "linear":
                    for adapter, weight in zip(adapters, weights):
                        ...
                        target_lora_B.data += current_adapter_lora_B.data * math.sqrt(
                            weight
                        )
                elif combination_type == "cat":
                    loras_A, loras_B = [], []
    ...
    # edit starts
    def enable_lora_gate(self, adapter_name: str = "default"):
        """
        Enable the LoRA gate for the given adapter.
        """
        for name, module in self.model.named_modules():
            if isinstance(module, LoraLayer):
                module.enable_gate[adapter_name] = True

    def disable_lora_gate(self, adapter_name: str = "default"):
        """
        Disable the LoRA gate for the given adapter.
        """
        for name, module in self.model.named_modules():
            if isinstance(module, LoraLayer):
                module.enable_gate[adapter_name] = False

    def enable_input_auto_encoder_training(self, adapter_name: str = "default", intermediate_dim: int = 128):
        for name, module in self.model.named_modules():
            if isinstance(module, LoraLayer):
                dtype = module.base_layer.weight.dtype
                device = module.base_layer.weight.device
                module.input_auto_encoder[adapter_name] = nn.Sequential(
                    nn.Linear(module.in_features, intermediate_dim),
                    nn.ReLU(),
                    nn.Linear(intermediate_dim, module.in_features)
                ).to(device=device, dtype=dtype)

    def disable_input_auto_encoder_training(self, adapter_name: str = "default"):
        for name, module in self.model.named_modules():
            if isinstance(module, LoraLayer):
                module.input_auto_encoder[adapter_name] = nn.Identity()

    def gather_input_auto_encoder_loss(self):
        total_loss = None
        # num_auto_encoder = 0
        for name, module in self.model.named_modules():
            if isinstance(module, LoraLayer):
                ...
                if total_loss is None:
                    total_loss = module.auto_encoder_loss
                else:
                    total_loss += module.auto_encoder_loss
        return total_loss

    def sample_embedding_dict(self):
        ret = {}
        for name, module in self.model.named_modules():
            if isinstance(module, LoraLayer):
                ret[f"{name}.sample_embedding"] = torch.cat(module.sample_embedding_cache, dim=0).cpu()

        return ret

    def enable_sample_embedding_cache(self):
        for name, module in self.model.named_modules():
            if isinstance(module, LoraLayer):
                module.enable_sample_embedding_cache = True
